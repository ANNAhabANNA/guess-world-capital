...
def game_intro():
    ...
    instructions = [
        ...
        emoji.emojize(":index_pointing_up:") +
...
def ask_user():
    ...
    try:
        ...
        print(colored(("Check if you typed 'yes' or 'no'"), "red"))
        return ask_user()
...
def continent_choice():
    ...
    try:
        ...
        if chosen_continent == list(continents.keys())[0]:
            ...
            raise Exception("only the numbers from the " +
                            "list provided are considered.")
    except Exception as e:
        print(colored((f"\nThis is invalid input as {e}" +
                       " Please try again.\n"),
              "red"))
...
def play():
    ...
    while guess_num <= max_guess:
        ...
        if user_attempt.lower() == capital.lower():
            print(colored(("\nC'EST MAGNEFIQUE!"), "green") + " " +
                  "You are a force to be reckoned with in geography.")
            correct += 1
            total = correct + incorrect
            print(f"\nYour correct answers score: ", colored((correct),
                  "green"))
